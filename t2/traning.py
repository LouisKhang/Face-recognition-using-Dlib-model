...
class Ui_ChupanhvaTraning(object):
    def setupUi(self, QLSVWindow):
        QLSVWindow.setObjectName("QLSVWindow")
        QLSVWindow.resize(800, 600)

        self.centralwidget = QtWidgets.QWidget(parent=QLSVWindow)
        ...
        self.lineEdit.setPlaceholderText("Nhập tên người dùng")
        ...
        QLSVWindow.setCentralWidget(self.centralwidget)
        self.menubar = QtWidgets.QMenuBar(parent=QLSVWindow)
        QLSVWindow.setMenuBar(self.menubar)
        self.statusbar = QtWidgets.QStatusBar(parent=QLSVWindow)
        QLSVWindow.setStatusBar(self.statusbar)

        self.retranslateUi(QLSVWindow)

        self.load_config()

        self.face_detector = dlib.get_frontal_face_detector()
        self.save_queue = Queue()
        self.save_thread = threading.Thread(target=self.save_image_thread)
    ...
    def retranslateUi(self, QLSVWindow):
        _translate = QtCore.QCoreApplication.translate
        QLSVWindow.setWindowTitle(_translate("QLSVWindow", "Quản lý sinh viên"))
    ...
    def load_config(self):
        ...
        try:
            ...
            print("[WARNING] Invalid JSON in config.json. Using default configuration.")
            self.config = default_config

        os.makedirs(self.config['dataset_path'], exist_ok=True)

    def load_students(self):
        self.cursor.execute("SELECT ma_sv, ten FROM students")
        students = self.cursor.fetchall()
        self.student_dict = {student.ten: student.ma_sv for student in students}
        self.student_combo.addItems(self.student_dict.keys())
    ...
    def capture_images(self):
        ...
        if not face_name:
            QtWidgets.QMessageBox.warning(self.centralwidget, "Cảnh báo", "Vui lòng nhập tên người dùng trước khi chụp ảnh!")
            return
    ...
    def show_completion_message(self):
        QtWidgets.QMessageBox.information(self.centralwidget, "Hoàn thành", "Quá trình chụp ảnh hoàn tất!")
    ...
    def train_model_thread(self):
        ...
        self.face_recognizer = dlib.face_recognition_model_v1('C:\\Users\\PC\\Desktop\\t2_02\\t2\\shape_predictor_68_face_landmarks.dat')
        self.face_detector = dlib.get_frontal_face_detector()
        self.shape_predictor = dlib.shape_predictor('C:\\Users\\PC\\Desktop\\t2_02\\t2\\dlib_face_recognition_resnet_model_v1.dat')
...
def main():
    ...
    QLSVWindow = QtWidgets.QMainWindow()
    ui = Ui_ChupanhvaTraning()
    ui.setupUi(QLSVWindow)
    QLSVWindow.show()
    sys.exit(app.exec())
